...
public class JavaDBAccountRepository implements AccountRepository {
    private final static String DB_TABLE = "accounts";


    public Account save(Account val) throws Exception {
        val.setAccountStatus(AccountStatus.ACTIVE);
        String SQL = String.format("INSERT INTO %s (name, AccountStatus) VALUES ('%s', '%s')", DB_TABLE,
    ...
    public void deleteById(Long id) throws Exception {
        String SQL = String.format("delete from %s where idAccount=%s", DB_TABLE, id);
        DatabaseHandler.getStatement(SQL).execute();
    }

    public Account getByID(Long id) throws Exception {
        String SQL = String.format("select* from %s where idAccount=%s order by idAccount", DB_TABLE, id);
        return sqlToAccount(DatabaseHandler.getStatement(SQL).executeQuery());
    }

    public List<Account> getAll() throws Exception {
        String SQL = String.format("select* from %s order by idAccount", DB_TABLE);
        return sqlToAccounts(DatabaseHandler.getStatement(SQL).executeQuery());
    }

    public Account update(Account val) throws Exception {
        String SQL = String.format("UPDATE %s SET name = '%s' where idAccount =%s", DB_TABLE, val.getName(), val.getId());
